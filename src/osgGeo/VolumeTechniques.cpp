...
static char volume_tf_undef_frag_depth[] =
...
"    while(num_iterations>0.5)\n"	// bugfix to solve numerical instability
"    {\n"
// begin SEPARATE UNDEF CHANNEL modification
"        vec2 src = texture3D( baseTexture, texcoord ).ra;\n"
"        float udf = 1.0 - src[1];\n"
"        if ( udf<1.0 )\n"
"            src[0] = (src[0] - udf) / (1.0-udf);\n"
"\n"
"        vec4 color = texture1D( tfTexture, src[0]*tfScale+tfOffset );\n"
"        vec4 udfcol = texture1D( tfTexture, tfScale+tfOffset );\n"
"\n"
"        if ( udf >= 1.0 )\n"
"            color = udfcol;\n"
"        else if ( udf > 0.0 )\n"
...
"        }\n"
// end SEPARATE UNDEF CHANNEL modification
"\n"
"        float r = color[3]*TransparencyValue;\n"
"        if (r>AlphaFuncValue)\n"
"        {\n"
...
const char* RayTracedTechnique::volumeTfUndefFragDepthCode()
{
    return volume_tf_undef_frag_depth;
}
